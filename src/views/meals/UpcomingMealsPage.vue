  ...
  <ion-content>
    ...
    <section class="py-6 px-4">
      ...
      <div class="flex items-center mb-4">
        <ion-icon :icon="chevronBackCircle" color="primary" class="text-5xl cursor-pointer" @click="handleWeekBack" />
        <div class="flex-1 text-center font-bold">
          {{ currentWeek.start }} - {{ currentWeek.end }}
        </div>
        <ion-icon :icon="chevronForwardCircle" color="primary" class="text-5xl cursor-pointer" @click="handleWeekForward" />
      </div>
...
import { IonContent, IonIcon, IonCard, IonCardContent, IonButton } from '@ionic/vue'
...
const weekMeals = computed(() => mealStore.mealsForDateRange(startOfWeekDate.value, endOfWeekDate.value))
...
function handleWeekBack() {
  const newStart = startOfWeek(new Date(startOfWeekDate.value.setDate(startOfWeekDate.value.getDate() - 7)), { weekStartsOn: 1 })
  const newEnd = endOfWeek(new Date(endOfWeekDate.value.setDate(endOfWeekDate.value.getDate() - 7)), { weekStartsOn: 1 })
